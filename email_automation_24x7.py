...
class EmailAutomation24x7:
    def __init__(self, config_file: str = "config.json"):
        ...
        self.optimal_interval = (
            24 * 60 * 60
        ) // self.daily_limit  # seconds between emails
        print(
            f"📊 Optimal interval: {self.optimal_interval} seconds (~{self.optimal_interval/60:.1f} minutes) between emails"
        )
    ...
    def load_hr_contacts(self) -> pd.DataFrame:
        ...
        try:
            ...
            if "email" not in df.columns:
                raise ValueError(
                    "Required column 'email' not found in HR contacts file"
                )
    ...
    def connect_smtp(self) -> bool:
        ...
        try:
            ...
            self.logger.error(
                f"SMTP connection failed (attempt {self.connection_attempts}): {e}"
            )

            if self.connection_attempts < self.max_connection_attempts:
                wait_time = min(
                    300, 2**self.connection_attempts * 60
                )  # Cap at 5 minutes
    ...
    def calculate_smart_delay(self) -> int:
        ...
        if self.sent_today >= self.daily_limit:
            ...
            self.logger.info(
                f"Daily limit reached ({self.daily_limit}). Waiting {wait_seconds/3600:.1f} hours until next day."
            )
        ...
        if remaining_emails > 0:
            ...
            self.logger.info(
                f"Progress: {self.sent_today}/{self.daily_limit} emails sent today"
            )
            self.logger.info(
                f"Remaining: {remaining_emails} emails in {remaining_seconds/3600:.1f} hours"
            )
    ...
    def send_email(self, contact: Dict, template: str) -> bool:
        ...
        try:
            ...
            self.sent_today += 1
            self.logger.info(
                f"Email sent successfully to {contact['email']} ({contact.get('company', 'Unknown Company')})"
            )
            self.logger.info(
                f"Daily progress: {self.sent_today}/{self.daily_limit} emails sent"
            )
            ...
            if delay > 60:  # If delay is more than 1 minute, show countdown
                self.logger.info(
                    f"Next email in {delay} seconds ({delay/60:.1f} minutes)"
                )
            else:
    ...
    def get_unsent_contacts(self, contacts_df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            self.logger.info(
                f"Found {len(unsent_contacts)} unsent contacts out of {len(contacts_df)} total"
            )
    ...
    def run_continuous(self):
        ...
        self.logger.info(
            f"Target: Complete 1,838 contacts in ~{1838/self.daily_limit:.1f} days"
        )
            ...
            while True:
                try:
                    ...
                    if unsent_contacts.empty:
                        self.logger.info(
                            "🎉 All emails have been sent! Campaign completed."
                        )
                        break
                    ...
                    if self.sent_today >= self.daily_limit:
                        ...
                        wait_seconds = (next_day - now).total_seconds()
                        self.logger.info(
                            f"Daily limit reached. Sleeping until next day ({wait_seconds/3600:.1f} hours)"
                        )
                    ...
                    if not success:
                        ...
                        self.logger.info(
                            "Email sending failed. Waiting 5 minutes before retry..."
                        )
                        time.sleep(300)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="HR Cold Email Automation System - 24x7 Version"
    )
    parser.add_argument(
