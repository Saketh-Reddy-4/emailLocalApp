        ...
        try:
            ...
            for col in required_columns:
                if col not in df.columns:
                    raise ValueError(
                        f"Required column '{col}' not found in HR contacts file"
                    )
...
<body>
    ...
    <div class="signature">
        ...
        {email_address}</p>
            ...
            self.logger.info(
                f"Created default email template: {self.config['email_template_file']}"
            )
    ...
    def is_business_hours(self) -> bool:
        ...
        country_holidays = holidays.country_holidays(
            self.config.get("country_code", "IN")
        )
    ...
    def send_email(self, contact: Dict, template: str) -> bool:
        ...
        try:
            ...
            delay = random.randint(
                self.config.get("min_delay", 30), self.config.get("max_delay", 120)
            )
    ...
    def run_batch(self, batch_size: int = 50):
        ...
        try:
            ...
            self.logger.info(
                f"Batch completed: {success_count}/{len(batch_contacts)} emails sent successfully"
            )
...
def main():
    ...
    parser.add_argument(
        "--batch-size", type=int, default=50, help="Number of emails per batch"
    )
    parser.add_argument(
