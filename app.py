...
DEFAULT_TEMPLATE = """Hi {{hr_name}},

I hope you’re doing well.

I’m writing to express my strong interest in the Salesforce opportunity. I have 5+ years of experience in Salesforce development, with hands-on experience in Lightning Web Components (LWC), Apex, Sales Cloud, Service Cloud, Financial Services Cloud (FSC), CPQ, and integrations.

I’ve worked on building and enhancing scalable Salesforce solutions, automation, and custom functionality in enterprise environments.

Please let me know a convenient time for a brief discussion regarding this opportunity.

Best regards,
Saketh Reddy Chintala
+1 571 541 6433 | [sakethredd04@gmail.com](mailto:sakethredd04@gmail.com)
...
def ensure_files() -> None:
    ...
    if not CONFIG_PATH.exists():
        CONFIG_PATH.write_text(
            "{\n"
            '  "candidate_name": "Saketh Reddy Chintala",\n'
            '  "candidate_phone": "+1 571 541 6433",\n'
            '  "candidate_email": "your_email@example.com",\n'
            '  "linkedin_url": "https://www.linkedin.com/in/your-profile/",\n'
            '  "default_subject": "Salesforce Talent - Saketh Resume"\n'
            "}\n",
            encoding="utf-8",
        )
...
def render_body(
    template: str, cfg: dict, hr_name: str, company: str, position: str
) -> str:
    replacements = {
        "{{hr_name}}": (hr_name or "").strip() or "Hiring Team",
        "{{company}}": (company or "").strip(),
        "{{position}}": (position or "").strip(),
        "{{candidate_name}}": str(cfg.get("candidate_name", "")),
        "{{candidate_phone}}": str(cfg.get("candidate_phone", "")),
        "{{candidate_email}}": str(cfg.get("candidate_email", "")),
        "{{linkedin_url}}": str(cfg.get("linkedin_url", "")),
    }
...
def render_single_tab(config: dict, template: str) -> None:
    ...
    subject = st.text_input(
        "Subject", value=str(config.get("default_subject", "Salesforce Talent - Saketh Resume"))
    )
    ...
    st.text_area("Email preview", value=body, height=240)
...
def render_batch_tab(config: dict, template: str) -> None:
    ...
    for index, row in enumerate(df.to_dict(orient="records"), start=1):
        ...
        subject = str(config.get("default_subject", "Salesforce Talent - Saketh Resume"))
        body = render_body(template, config, hr_name, company, position)
